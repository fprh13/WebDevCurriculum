...
> AWS API Gateway는 RESTful API를 쉽게 만들고 관리할 수 있도록 도와주는 서비스입니다. API Gateway를 통해 사용자는 HTTP 엔드포인트를 통해 엑세스할 수 있는 클라우드 서비스, 서버리스 함수 및 AWS Lambda 기반의 마이크로서비스를 손쉽게 만들고 배포할 수 있습니다.
...
> 1. **인증 및 권한 부여** : AWS IAM을 통해 API 호출자를 인증하고 권한 부여할 수 있습니다. 또한 서드파티 인증 공급자와 통합하여 OAuth, OpenID Connect 등의 인증 방식을 사용할 수도 있습니다.

> 2. **요청 및 응답 변환** : API Gateway는 클라이언트와 백엔드 서비스 간의 요청 및 응답 변환을 지원합니다. 예를 들어, JSON, XML, HTML 등 다양한 데이터 형식으로 요청을 받을 수 있고, 백엔드 서비스가 반환하는 데이터를 클라이언트가 요구하는 형식으로 변환할 수 있습니다.

> 3. **캐싱** : API Gateway는 클라이언트 요청에 대한 응답을 캐싱하여 응답 시간을 개선할 수 있습니다.

> 4. **로깅 및 모니터링** : API Gateway는 클라이언트 요청 및 응답 로깅을 제공합니다. 또한 AWS CloudWatch와 통합하여 API의 성능, 가용성 등을 모니터링할 수 있습니다.
